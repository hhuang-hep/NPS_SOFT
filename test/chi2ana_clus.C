...
// the chi2 threshold for waveform analysis. Clustering is done with
// ARS-analyzed information.
// 
// ** A slight memory has been detected. To be investigated...
//
// C. Munoz (13 Mar 2004)
//
/////////////////////////////////////////////////////////////////////
...
void ana()
{
  ...
  TH1F **hnpulmax[parts];
  TH1F *hblock[nchi2];
  //  Double_t chi2[2]={0.9,300.};
  //  Double_t chi2[9]={0.9,1.5,2.5,4.1,6.8,11.2,18.4,30.3,50.};
  Double_t chi2[9]={1.5,4.1,11.2,30.3,82.4,224.,609.,1656.,4501.};
  //Double_t chi2[4]={1.5,11.2,82.4,609.}; 
  for(Int_t i=0;i<parts;i++){

    he[i]=new TH1D*[nchi2];
    hphi[i]=new TH1D*[nchi2];
    htheta[i]=new TH1D*[nchi2];
    hmmsq[i]=new TH1D*[nchi2];
    hnpulave[i]=new TH1F*[nchi2];
    hnpulmax[i]=new TH1F*[nchi2];
   for(Int_t j=0;j<nchi2;j++){
  ...
  Double_t eff[nchi2],effbords[nchi2],effmm[nchi2],effene[nchi2],effclus[nchi2];
  Int_t goodclus;
  
  /////////Put your code here/////////////////////////

  for(Int_t chi=0;chi<nchi2;chi++){  
    cout<<chi<<"/"<<nchi2<<endl;
    cout<<"===================================="<<endl;
    cout<<"===================================="<<endl;
    cpgood=0.,cpbords=0.,cpmm=0.,cpene=0.,cpclus=0.;
    Double_t chi2thres=chi2[chi];
	...
	for(Int_t t=0;t<3;t++){//Loop on trees
      ...
      }//End loop on event blocks
      //caloev->Print();
      //            caloev->TriggerSimulation();
      //      cout<<"after"<<endl;
      //      caloev->Print();
      caloev->DoClustering();
      //cout<<"afteer clu"<<endl;
      //caloev->Print();
      // End of doing clustering///////////////////////////////////
...
//       if(good==kTRUE) cout<<"good "<<goodclus<<endl;
//       if(good==kFALSE) cout<<"false "<<endl;
...
//	    cout<<wave<<endl;
	...
	hmmsq[Int_t((x1+13.5)/6.75)+4*Int_t((-y1+15)/7.5)][chi]->
	  Fill((eini+pini-electron-(dvcsev->GetPhoton(goodclus))).Mag2());
	he[Int_t((x1+13.5)/6.75)+4*Int_t((-y1+15)/7.5)][chi]->
	  Fill(1.058*energy-mcevent->GetPhoton()->E());
	hnpulave[Int_t((x1+13.5)/6.75)+4*Int_t((-y1+15)/7.5)][chi]->Fill(npulseave/clus->GetClusSize());
	hnpulmax[Int_t((x1+13.5)/6.75)+4*Int_t((-y1+15)/7.5)][chi]->Fill(npulsemax);
	...
	if((dvcsev->GetPhoton(goodclus)).P()>0){
	  ...
	  hphi[Int_t((x1+13.5)/6.75)+4*Int_t((-y1+15)/7.5)][chi]->Fill(phi-phigen);	
	  htheta[Int_t((x1+13.5)/6.75)+4*Int_t((-y1+15)/7.5)][chi]->Fill(theta-thetagen);
...
// 	  cout<<-(dvcsev->GetPhoton(goodclus)).Px()<<" "<<(dvcsev->GetPhoton(goodclus)).P()<<" "<<-(mcevent->GetPhoton())->Px()<<" "<<mcevent->GetPhoton()->P()<<endl<<endl;
    ...
    eff[chi]=100.*cpgood/nevent;
    effbords[chi]=100.*cpbords/nevent;
    effmm[chi]=100.*cpmm/nevent;
    effclus[chi]=100.*cpclus/nevent;
  }//Loop on chi2

  TCanvas **c[6];
  gStyle->SetCanvasBorderMode(0);
  Int_t n=16,col=4,row=4;
  can->Print("resultsclus.ps[");

  Double_t maxe=0.,maxphi=0.,maxtheta=0.,maxmm=0.,maxpulseave=0.,maxpulsemax=0.;
  for(Int_t i=0;i<16;i++){
    for(Int_t k=0;k<nchi2;k++){
  ...
  Double_t X[nchi2],YE[parts][nchi2],Yphi[parts][nchi2],Ytheta[parts][nchi2],Ymm[parts][nchi2];
  for(Int_t j=0;j<6;j++){
    c[j]=new TCanvas*[nchi2];
    for(Int_t k=0;k<nchi2;k++){
      X[k]=chi2[k];
      c[j][k]=new TCanvas();
	...
	if(j==5) {
      ...
      c[j][k]->Print("resultsclus.ps");
    }//End k=0...nchi2
  }//End j=0...4
  c[3][0]->Print("resultsclus.ps]");

  TGraph **g[parts];
  for(Int_t i=0;i<parts;i++){
    g[i]=new TGraph*[4];
    g[i][0]=new TGraph(nchi2,X,YE[i]);
    g[i][1]=new TGraph(nchi2,X,Ytheta[i]);
    g[i][2]=new TGraph(nchi2,X,Yphi[i]);
    g[i][3]=new TGraph(nchi2,X,Ymm[i]);
  }
  can->Print("summaryclus.ps[");
  TCanvas *d[4];
  for(Int_t i=0;i<4;i++){
    d[i]=new TCanvas();
    d[i]->Divide(4,4);
    ...
    d[i]->Print("summaryclus.ps");
  }
  TGraph *geff=new TGraph(nchi2,X,eff);
  geff->SetMarkerColor(kRed);
  geff->SetLineColor(kRed);
  TGraph *geffbords=new TGraph(nchi2,X,effbords);
  geffbords->SetMarkerColor(kBlue);
  geffbords->SetLineColor(kBlue);
  TGraph *geffmm=new TGraph(nchi2,X,effmm);
  geffmm->SetMarkerColor(kGreen);
  geffmm->SetLineColor(kGreen);
  TGraph *geffclus=new TGraph(nchi2,X,effclus);
  ...
  ceff->Print("summaryclus.ps");
  cblocks->Print("summaryclus.ps");
