...
{  
  ...
  TFile *f=new TFile("treebruitonlyinshowerwph.root");
  ...
  t->SetFirstWindow(-100,70,"data");
  t->SetSecondWindow(-2,2,-100,70,"data");
...
//This is the tree to write
  TFile tf("treeonlyinshower.root","RECREATE");
  ...
  tree2.Branch("event_dvcs2.","TDVCSEvent",&dvcsev,16000,2);
...
//   hwf->Draw();
tf.Write();
  TFile ntu("tnuple6.root","RECREATE");
  ntuple->Write();
}
